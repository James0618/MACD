...
class DreamerMemory:
    def __init__(self, capacity, sequence_length, action_size, obs_size, n_agents, device, env_type,
                 central_obs_size=None):
        self.capacity = capacity
        self.sequence_length = sequence_length
        self.action_size = action_size
        self.obs_size = obs_size
        if central_obs_size is None:
            self.central_obs_size = obs_size
        else:
            self.central_obs_size = central_obs_size
    ...
    def init_buffer(self, n_agents, env_type):
        ...
        self.central_observations = np.empty((self.capacity, n_agents, self.central_obs_size), dtype=np.float32)
    ...
    def append(self, obs, action, reward, done, fake, last, av_action, central_obs=None):
        ...
        for i in range(len(obs)):
            ...
            if central_obs is not None:
                self.central_observations[self.next_idx] = central_obs[i]
            else:
                self.central_observations[self.next_idx] = obs[i]
    ...
    def get_transitions(self, idxs):
        ...
        central_observation = self.process_batch(self.central_observations, vec_idxs, batch_size)[1:]
        ...
        return {'observation': observation, 'central_observation': central_observation, 'reward': reward,
                'action': action, 'done': done, 'fake': fake, 'last': last, 'av_action': av_action}
