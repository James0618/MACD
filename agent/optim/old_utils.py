...
def rec_loss_with_consistence_loss(decoder, z, x, fake):
    x_pred, feat = decoder(z)
    batch_size = np.prod(list(x.shape[:-1]))
    gen_loss1 = (F.smooth_l1_loss(x_pred, x, reduction='none') * fake).sum() / batch_size
    return gen_loss1, feat
...
def compute_return_dae(reward, dae_reward, value, discount, bootstrap, lmbda, gamma):
    batch_size = reward.shape[0]
    values = torch.cat([value, bootstrap[None]], 0)
    deltas = reward - 0.5 * dae_reward + gamma * discount * values[1:] - values[:-1]
    adv_outputs, return_outputs = [], []
    accumulated_adv = values[-1]
    accumulated_return = reward[-1]
    for t in reversed(range(batch_size)):
        discount_factor = discount[t]
        accumulated_adv = deltas[t] + gamma * discount_factor * accumulated_adv * lmbda
        accumulated_return = reward[t] + gamma * discount_factor * accumulated_return
        adv_outputs.append(accumulated_adv)
        return_outputs.append(accumulated_return)
    advs = torch.flip(torch.stack(adv_outputs), [0])
    returns = torch.flip(torch.stack(return_outputs), [0])
...
def state_divergence_loss(prior, posterior, config, reduce=True, balance=0.2):
    ...
    post = kl_div_categorical(post_dist, prior_dist.detach())       # loss_rep
    pri = kl_div_categorical(post_dist.detach(), prior_dist)        # loss_dyn
