...
class DreamerModel(nn.Module):
    def __init__(self, config):
        ...
        if config.RECONSTRUCT_CENTRAL:
            self.observation_decoder = Decoder(embed=config.FEAT, hidden=config.HIDDEN, out_dim=config.CENTRAL_OBS_SIZE,
                                               activation=config.activation, norm=config.norm)
        else:
            self.observation_decoder = Decoder(embed=config.FEAT, hidden=config.HIDDEN, out_dim=config.IN_DIM,
                                               activation=config.activation, norm=config.norm)

        self.transition = RSSMTransition(config, config.MODEL_HIDDEN, activation=config.activation, norm=config.norm)
        ...
        self.reward_model = RewardModelV3(config, activation=config.activation, norm=config.norm)
        self.pcont = DenseBinaryModel(config.FEAT, 1, config.PCONT_LAYERS, config.PCONT_HIDDEN)

        if config.ENV_TYPE == Env.STARCRAFT or config.ENV_TYPE == Env.MPE:
