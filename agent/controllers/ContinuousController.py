...
class ContinuousController:
    def __init__(self, config):
        ...
        self.actor = ContinuousActor(config.FEAT, config.ACTION_SIZE, config.ACTION_HIDDEN, config.ACTION_LAYERS,
    ...
    def dispatch_buffer_ac(self, done_flag):
        total_buffer = {k: np.asarray(v, dtype=np.float32) for k, v in self.buffer.items()}
        last = np.zeros_like(total_buffer['done'])
        if done_flag:
            last[-1] = 1.0
    ...
    @torch.no_grad()
    def step(self, observations, avail_actions, nn_mask, deterministic=False):
        """"
        ...
        action, log_probs, action_mean, action_std = self.actor(feats, deterministic=deterministic)
        assert avail_actions is None
